...
When the z-score exceeds 3.0, a 14-day (≈10 trading day) entry block is placed on that symbol. The block is stored as blocked_until date and checked before any new entry. If the Reddit data source is degraded, the M3 composite threshold is increased by 20% (via `m3_threshold_multiplier`), making entries more selective — but the existing M6 block is not automatically lifted.
