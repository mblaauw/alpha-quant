...
Alpha-Quant requires a read-only dashboard for monitoring: equity curve, position table, risk map, daily journal, and concept card browser. The dashboard reads from the SQLite state store and event log only — zero coupling to pipeline internals. No user interaction that affects the running system.
...
- Zero coupling: consumes only persisted data (SQLite events, positions, equity curve)
...
- **Option C: Grafana** — Industry standard for monitoring dashboards, but requires Prometheus/InfluxDB metrics pipeline; overkill for reading SQLite directly
...
2. Zero coupling: reads SQLite via SQLAlchemy Core (the same dependency already used by the pipeline)
3. Caching with `@st.cache_data` means the dashboard does not re-read the SQLite file on every 60-second auto-refresh unless data changed
...
- Shares the same data access code as the pipeline (SQLAlchemy Core models)
