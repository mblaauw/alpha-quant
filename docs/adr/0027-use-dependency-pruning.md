...
- `apscheduler` → moved to optional `[scheduler]` extra (core library retained for scheduler module)
...
- APScheduler must be installed separately if running the scheduler daemon: `uv sync --extra scheduler`
