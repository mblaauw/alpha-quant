...
class DuckDBEventSink(EventSink):
    """Event sink backed by DuckDB, suitable for the shared state database."""

    def __init__(self, db_path: str | Path = Path("data") / "state.db") -> None:
        self._db_path = Path(db_path)
        self._conn = duckdb.connect(str(self._db_path))
        self._conn.execute(
            """
            CREATE TABLE IF NOT EXISTS event_sink_log (
                id TEXT PRIMARY KEY,
                run_id TEXT,
                event_type TEXT,
                ts TEXT,
        ...
        self._conn.execute("CREATE INDEX IF NOT EXISTS idx_esl_run_id ON event_sink_log(run_id)")
        self._conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_esl_event_type ON event_sink_log(event_type)"
        )
        self._conn.execute("CREATE INDEX IF NOT EXISTS idx_esl_ts ON event_sink_log(ts)")

    @override
    def emit(self, event: DomainEvent) -> None:
        payload = event.model_dump_json()
        self._conn.execute(
            "INSERT INTO event_sink_log (id, run_id, event_type, ts, payload_json, source)"
            " VALUES (?, ?, ?, ?, ?, ?)",
            [
                event.event_id,
                event.run_id,
    ...
    def query(
        ...
        rows = self._conn.execute(
            f"SELECT payload_json FROM event_sink_log WHERE {where} ORDER BY ts",
            params,
        ).fetchall()
